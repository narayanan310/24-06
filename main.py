...
def _resolve_clause(
    ...
    regex: RegexIntentResolver, domain_res: DomainResolver,
    rag, slm: SLMIntentResolver, slm_ok: bool,
    domain: str | None = None,
) -> dict | None:
    # Stage 1: Context window (relative adjustments, e.g. make it colder)
    intent = ctx_win.resolve_relative(text, domain=domain)
    if intent: return intent

    # Stage 2: Context resolver (state-aware resolution)
    intent = ctx_res.resolve(text)
    if intent: return intent

    # Stage 3: Regex — fast deterministic pattern matching
    intent = regex.resolve(text)
    if intent: return intent

    # Stage 4: Domain resolver — semantic/metaphorical expressions
    #           (inserted per PRODUCTION_NOTES.txt recommendation)
    intent = domain_res.resolve(text)
    if intent:
        print(f"[System] DomainResolver matched: {intent.get('command')} → {intent.get('value')}")
        return intent

    # Stage 5: RAG — knowledge-base lookup (manual / how-to queries)
    ...
    # Stage 6: SLM — LLM fallback (last resort only)
    if slm_ok:
    ...
    while True:
        ...
        if dlg.is_repair(raw):
            ...
            if action == "cancel_pending":
                ...
                # resolve_repair could not determine what to fix — try re-resolving as a fresh command
                intent = _resolve_clause(raw, ctx_win, ctx_res, regex_eng, domain_res, rag_eng, slm_eng, slm_ready)
                if intent:
                    await _exec(raw, intent)
                else:
                    _say("I'm not sure what you'd like to change. Could you be more specific?")
            else:
                intent = _resolve_clause(raw, ctx_win, ctx_res, regex_eng, domain_res, rag_eng, slm_eng, slm_ready)
        ...
        for clause in clauses:
            intent = _resolve_clause(clause, ctx_win, ctx_res, regex_eng, domain_res, rag_eng, slm_eng, slm_ready, domain=domain_hint)
